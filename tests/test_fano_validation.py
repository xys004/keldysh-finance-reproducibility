...
from keldysh_finance.fano_validation import (expected_recentered_joint_second_moment,
...
def test_recentered_magnitude_expectation_matches_exact_enumeration():
    flow = np.array([1.0, -2.0, 2.5, 1.5, -3.0, -0.5])
    strata = np.array([0, 1, 0, 1, 0, 1])
    analytic = expected_recentered_magnitude_second_moment(
        flow, strata, window=2
    )

    signs = np.sign(flow)
    magnitudes = np.abs(flow)
    groups = [np.flatnonzero(strata == h) for h in (0, 1)]
    enumerated = []
    for p0 in permutations(range(3)):
        for p1 in permutations(range(3)):
            surrogate = np.empty_like(flow)
            for pos, order in zip(groups, (p0, p1)):
                surrogate[pos] = signs[pos] * magnitudes[pos[list(order)]]
                surrogate[pos] -= surrogate[pos].mean()
            enumerated.append(surrogate.reshape(-1, 2).sum(axis=1) ** 2)
    expected = np.mean(enumerated, axis=0)
    assert np.allclose(analytic, expected, rtol=0, atol=1e-14)


def test_recentered_magnitude_expectation_rejects_repeated_strata_in_window():
...
def test_recentered_joint_expectation_matches_exact_enumeration():
    flow = np.array([1.0, 10.0, 2.0, 20.0, 4.0, 40.0])
    strata = np.array([0, 1, 0, 1, 0, 1])
    analytic = expected_recentered_joint_second_moment(flow, strata, window=2)
    ...
    for p0 in permutations(range(3)):
        for p1 in permutations(range(3)):
            surrogate = np.empty_like(flow)
            for pos, order in zip(groups, (p0, p1)):
                surrogate[pos] = flow[pos[list(order)]]
                surrogate[pos] -= surrogate[pos].mean()
            enumerated.append(surrogate.reshape(-1, 2).sum(axis=1) ** 2)
    expected = np.mean(enumerated, axis=0)
    assert np.allclose(analytic, expected, rtol=0, atol=1e-13)

    diagonal = flow.reshape(-1, 2)
    diagonal = (diagonal ** 2).sum(axis=1).mean()
    assert analytic.mean() < diagonal


def test_recentered_joint_equals_diagonal_after_matching_demeaning():
    flow = np.array([1.0, 10.0, 2.0, 20.0, 4.0, 40.0])
    strata = np.array([0, 1, 0, 1, 0, 1])
    centered = flow.copy()
    for h in (0, 1):
        pos = strata == h
        centered[pos] -= centered[pos].mean()
    analytic = expected_recentered_joint_second_moment(centered, strata, window=2)
    diagonal = (centered.reshape(-1, 2) ** 2).sum(axis=1)
    assert np.isclose(analytic.mean(), diagonal.mean(), rtol=0, atol=1e-14)
